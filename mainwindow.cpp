...
void MainWindow::CreateConnections() {
	...
	// other settings
	connect(ui_.sound_effect_checkbox, &QCheckBox::checkStateChanged,
			[this](int s) { sound_effect_ = (s == Qt::Checked); });
	connect(ui_.tray_popup_checkbox, &QCheckBox::checkStateChanged,
			[this](int s) { tray_popup_ = (s == Qt::Checked); });
	connect(ui_.start_minimize_checkbox, &QCheckBox::checkStateChanged,
...
void MainWindow::onStatusChangedNotification(Pomodoro::Status s) {
	...
	if (sound_effect_) {
		...
		effect->setSource(QUrl("qrc:/sounds/bell.wav"));
		effect->play();
	}
}
...
void MainWindow::onReminderPopup() {
	tray_icon_->showMessage(tr("Forget to set activity?"), tr("Set your current activity NOW!"));
}
